...
object Contracts {
    ...
    fun containsJustDigits(str: String?): Boolean {
        contract {
            // small DSL for testing things bout this function
    ...
    fun demoNullableString() {
        ...
        println("I have maybe string $maybeString")
    ...
    open class User(open val username: String, open val email: String) {
        ...
        fun isValidAdmin(): Boolean {
            ...
            return this is Admin && email.endsWith("@kotlinadmins.com")
        }
    }

    class Admin(override val username: String, override val email: String, val permissions: List<String>) :
        User(username, email) {
        fun purgeData() {
    ...
    fun attemptAdminTasks(user: User) {
        if (user.isValidAdmin()) { // if it is true => user is admin
    ...
    fun demoAdmin() {
        val admin = Admin("admin", "admin@kotlinadmins.com", listOf("READ", "WRITE"))
    ...
    fun <R : Resource, A> R.bracket(block: (R) -> A): A { // "bracket" pattern
        contract {
            callsInPlace(block, InvocationKind.EXACTLY_ONCE) // exactly once
        }
